...
{
$fps = validInt(getset('fps',ceil($event['Frames'] / $event['Length'])));
$ffparms = "-vcodec flv -r 25 -b:v 2048k -preset ultrafast ".$baseURL."/".$fname." 2>&1 2>/dev/null; echo $";
$shellCmd = getFfmpegPath()." -y -r ".$fps." -i ".$baseURL."/%0".ZM_EVENT_IMAGE_DIGITS."d-capture.jpg";
$shellCmd .= " ".$ffparms;
$shellOutput = shell_exec($shellCmd);
  if ($shellOutput == 0)
  {
  $url = $hostname.getEventPathSafe($event)."/".$fname;
  }

}
else
{
// FLV Video File already exists. No need to transcode. Just set URL to video.
$url = $hostname.getEventPathSafe($event)."/".$fname;
}

if (!file_exists(trim($fnameOut))) 
{
  //Error writing file. Display static error on FLV Player
  $url = "";
}
	...
	<table style="width:600px; margin:0 auto;">
	 <tr>
	  <td class="left"><span id="dataTime" title="<?= $SLANG['Time'] ?>"><?= strftime( STRF_FMT_DATETIME_SHORT, strtotime($event['StartTime'] ) ) ?></span></td>
        ...
        <div id="imageFeed">
        <object id="eventStream" type="application/x-shockwave-flash" data="<?=getSkinFile('swf/player_flv_maxi.swf');?>" width="640" height="480">
            <param name="movie" value="<?=getSkinFile('swf/player_flv_maxi.swf');?>" />
                <param name="allowFullScreen" value="true" />
                <param name="quality" value="low" />
                    <param name="FlashVars" value="flv=<?php echo $url?>&amp;autoplay=0&amp;autoload=1&amp;showiconplay=1&amp;volume=0&amp;showtime=1&amp;showfullscreen=1" />
        </object>
        </div>
       <div id="videoExport">
