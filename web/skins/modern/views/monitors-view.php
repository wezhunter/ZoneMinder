...
for ( $i = 0; $i < count($monitors); $i++ ) {
	...
	$tmpheight = $contentwidth * $monitors[$i]['Height']/2 / $monitors[$i]['Width'] ;
	if ( $maxHeight < $tmpheight ) $maxHeight = $tmpheight;
//	$maxHeight = 240
//	$maxHeight = $monitor['Width'] * ('.' . 50) + 20;
//	$maxHeight = $contentwidth * ($monitor['Height'] / ('.' . 50) + 20);
	    
	$monitors[$i]['connKey'] = generateConnKey();
	$monitors[$i]['index'] = $index++;

	if ($monitors[$i]['Id']!=4) $displayMonitors[$monitors[$i]['Id']] = $monitors[$i];
}
