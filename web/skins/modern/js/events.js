$(document).ready(function() {
	...
	$tabs = $("#tabs_events")
		.tabs({
			...
			load: function(event, ui) { $(".event").colorbox({rel:'event',iframe:true, innerWidth:800, innerHeight:600}); },
...
function dumpProps4(obj, parent) {
	...
	function FetchMore(){
		...
		$.post("skins/modern/includes/getEvents.php?is_graph_mode=0&MonitorName="+MonitorName+"&gridwidth="+$("#tabs_events").width()+"&page="+page, function(data){ // Get more events
			...
			$(".event").colorbox({rel:'event',iframe:true, innerWidth:800, innerHeight:600});
		});
		page = page + 1;
		hide_spinner();
	}
