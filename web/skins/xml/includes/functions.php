...
function getFfmpeg264FoutParms($br, $fout)
{
	...
	$ffparms .= " -vcodec libx264 -b:v ".$br;
	$ffparms .= " -flags +loop -partitions +parti4x4+partp8x8+partb8x8";
        $ffparms .= " -preset ultrafast -profile main";
        $ffparms .= "  -r 30000/1001";
	$ffparms .= " -rc_eq 'blurCplx^(1-qComp)' -qcomp 0.6";
	$ffparms .= " -qmin 10 -qmax 51";
	$ffparms .= " -analyzeduration 0 -async 2 ".$fout." 2> /dev/null";
	return $ffparms;
}
...
function kill264proc($monitor) {
	 ...
	 * should be properly validated before executing */
	$pid = trim(shell_exec("pgrep -f -x \"zmstreamer -m ".$monitor." -o avi -f 25 -b 800000\""));
	if ($pid == "") {
...
function stream264fn ($mid, $width, $height, $br) {
	$cdir = "./temp";
	$zmstrm = "zmstreamer -m ".$mid."  -o avi -f 25 -b 1000000 2> /dev/null";
	$ffstr = getFfmpeg264Str($width, $height, $br, "-", "-");
	$seg = "segmenter - ".ZM_EYEZM_SEG_DURATION." ".$cdir."/sample_".$mid." ".$cdir."/".m3u8fname($mid)." ../ 2> /tmp/segmenter.out";
